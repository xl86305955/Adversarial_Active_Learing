...
def train(X_train, Y_train, X_val, Y_val, params, num_epochs=50, batch_size=128, train_temp=1):
...
def query(X_test, Y_train, TASK, num_classes, ONE_HOT=True):
    tf.reset_default_graph()
    if TASK == 'mnist10':
...
def cnn_gen_adv(X_train, Y_train, ATTACK, TASK, image_size, num_channels, nb_classes):
